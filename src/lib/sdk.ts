...
class UniversalSDK {
  ...
  async sendEmail(to: string, subject: string, html: string, smtpOverride: SMTPConfig | null = null): Promise<boolean> {
    ...
    if (!endpoint) {
      console.warn('SMTP endpoint not configured, skipping email send');
  ...
  async testSMTPConnection(): Promise<boolean> {
    if (!this.smtp.test) return true; // Assume working if no test function
  ...
  async register(email: string, password: string, profile: Partial<User> = {}): Promise<User> {
    ...
    const user = await this.insert<User>("users", { email, password: hashed, ...profile, verified: !this.authConfig.requireEmailVerification });
  ...
  async listMediaFiles(tag: string = "", max: number = 30): Promise<any[]> {
    ...
    const res = await fetch(
      `https://api.cloudinary.com/v1_1/${this.cloudinary.cloudName}/resources/image?${query}`,
      {
        method: "GET",
      }
    );
  ...
  async init(): Promise<UniversalSDK> {
    // Initialize schemas for the platform
    ...
    this.setSchema('orders', {
      required: ['storeId', 'customerId', 'total'],
      types: {
        storeId: 'string',
        customerId: 'string',
        total: 'number',
